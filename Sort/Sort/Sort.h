...
//		希尔排序
//	对于插入排序而言：
//		其特点是，每一趟的单次排序会将待排序数组中的数据改造成一种趋向于局部有序的姿态
//		而其插入则是主要表现在：未接受排序数据插入到局部有序数据形成的有序数组中
// 
// 2、交换排序：
//		冒泡排序
//		快速排序~~~（将一个数组分成两份，要求分成的两个数组中，元素个数尽量相同，但是两个数组的总元素之和差要尽可能更大）
//	对于交换排序而言：
//		其特点是，每一趟单次排序之后，数组中就会有一个数据被放到数组中的正确位置。经过N次这样的放置，最终排序成功
//		而其交换主要表现在：将未能确定具体位置的元素经过大小比较之后确定其位置，并将其与其他未能确定位置的元素交换位置
// 
// 3、选择排序
//		简单选择排序
//		堆排序（大量数据M个中，找出前n个大小的数据）
//	特点：
// 
// 4、归并排序
// 
